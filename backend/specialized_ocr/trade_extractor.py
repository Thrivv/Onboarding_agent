...
Extracts 33 bilingual fields (English + Arabic) plus managers and partners lists
...
# Fields expected for Trade License extraction
ENGLISH_FIELDS = [
    # License Information
...
EXPECTED_FIELD_COUNT = len(ENGLISH_FIELDS)  # 33 fields
...
class TradeLicenseExtractor:
    """Trade License extractor with bilingual support and retry logic - Backend version"""
    ...
    @retry_with_backoff(max_retries=3, base_delay=10)
    def extract_with_ai(self, raw_text: str) -> Tuple[Dict, Dict, List[Dict], List[Dict]]:
        """Extract bilingual Trade License fields using AI with retry logic - ENHANCED PROMPT"""
        
        prompt = f"""You are an expert at extracting information from UAE Dubai Trade License (Commercial License) documents.
...
4. Extract manager and partner information separately as lists
5. Preserve all Arabic text exactly as shown
6. If a field is not found, set it as empty string ""
7. Extract ALL 33 base fields plus managers and partners arrays
...
- "issue_date" (تاريخ_الإصدار): Date Issue/تاريخ الإصدار
- "expiry_date" (تاريخ_الانتهاء): Date Expiry/تاريخ الانتهاء
...
- "activity_1", "activity_2", "activity_3": Business activities listed
...
English Fields (ALL 33 required):
{json.dumps(ENGLISH_FIELDS, indent=2)}

Arabic Fields (ALL 33 required):
...
MANAGERS EXTRACTION:
Extract all managers/officers as a separate list with:
...
- role: Position/role (e.g., "Manager", "Director")

PARTNERS/SHAREHOLDERS EXTRACTION:
Extract all partners/shareholders as a separate list with:
- person_number: Unique identifier
- name_english: Partner name in English
- name_arabic: Partner name in Arabic
- nationality_english: Nationality in English
- nationality_arabic: Nationality in Arabic
- share_percentage: Ownership percentage (e.g., "51.0000000%")

IMPORTANT: Extract ALL partners/shareholders from the document. If there are 5 partners, return 5 entries.

Example JSON structure with complete extraction:
{{
  "english": {{
    "license_number": "123456",
    "main_license_number": "654321",
    "commercial_register_number": "9876543",
    "issue_date": "01/01/2020",
    "expiry_date": "31/12/2022",
    "receipt_number": "98765432",
    "print_date": "15/06/2021 10:30",
    ...
    "activity_2": "Import Export",
    "activity_3": "Wholesale",
    "phone": "971-4-1234567",
    "fax": "971-4-1234568",
    ...
    "nominated_capital": "500,000",
    "paid_capital": "500,000",
    "currency": "UAE Dirhams",
    "number_of_shares": "100",
    "payment_voucher_number": "12345678",
    "payment_mode": "Online Payment",
    "payment_amount": "5,000"
  }},
  "arabic": {{
    "رقم_الرخصة": "123456",
    "رقم_الرخصة_الأم": "654321",
    "رقم_السجل_التجاري": "9876543",
    "تاريخ_الإصدار": "01/01/2020",
    "تاريخ_الانتهاء": "31/12/2022",
    "رقم_الإيصال": "98765432",
    "تاريخ_الطباعة": "15/06/2021 10:30",
    ...
    "الشكل_القانوني": "ذات مسئولية محدودة",
    "رقم_دنز": "123456789",
    "النشاط_1": "تجارة عامة",
    ...
    "رأس_المال_الاسمي": "500,000",
    "رأس_المال_المدفوع": "500,000",
    "العملة": "درهم إماراتي",
    "عدد_الأسهم": "100",
    "رقم_إذن_الدفع": "12345678",
    "كيفية_الدفع": "الدفع الإلكتروني",
    "مبلغ_الدفع": "5,000"
  }},
  "managers": [
    {{
      "person_number": "111111",
  ...
  "partners": [
    {{
      ...
      "share_percentage": "51.0000000%"
    }},
    {{
      "person_number": "333333",
      "name_english": "SARAH JOHNSON",
      "name_arabic": "سارة جونسون",
      "nationality_english": "United States",
      "nationality_arabic": "الولايات المتحدة",
      "share_percentage": "49.0000000%"
    }}
  ]
}}

Return ONLY valid JSON with ALL {EXPECTED_FIELD_COUNT} base fields populated, plus managers and partners arrays.
Extract ALL managers and ALL partners/shareholders from the document - do not limit the number.
"""
            ...
            if response and not response.startswith("API Error"):
                try:
                    ...
                    if json_match:
                        ...
                        print(f"[INFO] Trade License extraction complete: {len(partners)} partners, {len(managers)} managers")
                        
                        return english_data, arabic_data, managers, partners
                        
                except json.JSONDecodeError as e:
                    print(f"[ERROR] Trade License JSON parsing error: {e}")
                except Exception as e:
                    print(f"[ERROR] Trade License extraction failed: {e}")

        except Exception as e:
            print(f"[ERROR] Trade License API call failed: {e}")

        # ALWAYS return 4 values - even on failure
        print(f"[WARN] Trade License extraction failed, returning empty data")
        return (
    ...
    def post_process_data(self, data: Dict, raw_text: str) -> Dict:
        """Fix incomplete or missing values using regex fallback - ENHANCED"""
        incomplete_patterns = {
            "license_number": [
                r'License No[.:]?\s*(\d+)',
                r'Lic\. No[.:]?\s*(\d+)',
    ...
    def validate_extraction(self, english_data: Dict, arabic_data: Dict, 
                        managers: List[Dict], partners: List[Dict]) -> Dict:
        """Validate presence and completeness - RELAXED THRESHOLD"""
        validation = {
        ...
        # Count non-empty fields (activity_3 is optional, so 32 required)
        ...
        # Validation rules - CHANGED: Reduced from 33 to 28 fields (85% of fields)
        # 1. All critical fields must be present
        # 2. At least 28 out of 33 fields (activity_3 optional)
        # 3. Must have partners OR managers
        has_members = len(partners) > 0 or len(managers) > 0
        
        if validation["missing_fields"]:
            validation["is_valid"] = False
            print(f"[VALIDATION] Trade License INVALID: Missing critical: {validation['missing_fields']}")
        elif non_empty_english < 28:  # CHANGED from 29
            validation["is_valid"] = False
            validation["issues"].append(f"Insufficient fields: {non_empty_english}/33 (need 28+)")
            print(f"[VALIDATION] Trade License INVALID: Only {non_empty_english}/33 fields (need 28+)")
        elif not has_members:
            validation["is_valid"] = False
            validation["issues"].append("No partners or managers extracted")
            print(f"[VALIDATION] Trade License INVALID: No partners/managers found")
        else:
            validation["is_valid"] = True
            validation["issues"] = []
            print(f"[VALIDATION] Trade License VALID: {non_empty_english}/33 fields, {len(partners)} partners ✅")
