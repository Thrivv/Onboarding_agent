...
async def send_completion_email_endpoint(request: dict):
    ...
    ✅ Runs cross-validation + SurePass API validation ONCE
    ✅ Generates PDF summary using handle_reply.py function
    ...
    try:
        ...
        if document_confirmation_sent and onboarding_step == "verification_complete":
            print("[INFO] ⚠️ User already completed onboarding")
            return {
                "success": True,
        ...
        try:
            ...
            # ✅ STEP 2A: Send "Validation Pending" notification
            print("[INFO] 📧 Sending validation pending notification...")
            try:
                pending_subject = "⏳ Document Verification in Progress"
                pending_body = f"""
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2 style="margin: 0;">⏳ Verification in Progress</h2>
                        </div>
                        <div class="content">
                            <p>Dear {user_name},</p>
                            <p>We are currently validating your submitted documents. This process typically takes 30-60 seconds.</p>
                            <p>You will receive another email shortly with the validation results.</p>
                        </div>
                        <div class="footer">
                ...
                """
                send_email(email, pending_subject, pending_body, html=True)
                print("[SUCCESS] ✅ Validation pending email sent")
            except Exception as pending_error:
                print(f"[WARNING] ⚠️ Could not send pending email: {pending_error}")
            
            # ✅ STEP 2B: Run cross-validation
            print("[INFO] 🔍 Running cross-validation...")
            validation_result = validator.validate_documents(email, user)
            
            # ✅ STEP 2C: Save validation result as JSON
            ...
            # ============================================================================
            # 🔍 STEP 3: HANDLE CROSS-VALIDATION RESULT
            # ============================================================================
            if not validation_result["passed"]:
                ...
                mismatch_list = '<br>'.join([f"• {m}" for m in validation_result["mismatches"]])
                failure_subject = "❌ Document Validation Failed"
                failure_body = f"""
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h2 style="margin: 0;">❌ Validation Failed</h2>
                        </div>
                        <div class="content">
                            <p>Dear {user_name},</p>
                            <div class="error-box">
                                <h3 style="color: #721c24; margin-top: 0;">Issues Found:</h3>
                                <p style="color: #721c24;">{mismatch_list}</p>
                            </div>
                            <p>Please review your documents and re-upload corrected versions.</p>
                        </div>
                        <div class="footer">
                            <p style="margin: 0;">Best regards,</p>
                            <p style="margin: 5px 0 0 0; font-weight: 600;">Thrivv Onboarding Team</p>
                ...
                try:
                    send_email(email, failure_subject, failure_body, html=True)
                    print("[INFO] ✅ Validation failure email sent")
                except Exception as fail_email_error:
                    print(f"[WARNING] ⚠️ Could not send failure email: {fail_email_error}")
                ...
                return {
                    "success": False,
                    "message": f"Cross-validation failed. Please review:\n\n{error_list}",
                    "validation_passed": False,
                    "validation_errors": validation_result["mismatches"],
        ...
        # ============================================================================
        # ✅ STEP 4: SUREPASS API VALIDATION (if enabled)
        # ============================================================================
        print(f"\n{'='*80}")
        print(f"🔍 [SUREPASS] Running government verification...")
        print(f"{'='*80}")
        ...
        try:
            ...
            if not surepass_valid:
                print(f"❌ ERROR: SurePass validation failed: {surepass_message}")
                return {
                    "success": False,
        ...
        except Exception as surepass_error:
            print(f"[ERROR] ❌ SurePass error: {surepass_error}")
            return {
                "success": False,
                "message": f"Government verification error: {str(surepass_error)}",
                "validation_passed": False
            }
        ...
        # ============================================================================
        # ✅ STEP 6: GENERATE PDF AND SEND COMPLETION EMAIL
        # ============================================================================
        print(f"\n📧 [INFO] Generating PDF and sending completion email...")
        
        try:
            # ✅ Call the function from handle_reply.py to generate PDF and send email
            from llm_pipeline.handle_reply import send_completion_email
            
            print(f"[INFO] 📄 Calling send_completion_email from handle_reply.py...")
            ...
            import traceback
            traceback.print_exc()
            # Return error but mark as partially successful
            return {
                "success": True,
                "message": "Validation passed but email sending failed. User record updated.",
                "validation_passed": True,
                "email_failed": True,
        ...
        try:
            insert_conversation_log(
                user_email=email,
                role="agent",
                message=f"🎉 Document Verification Complete!\n\nUser confirmed all documents on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}.\n\n✅ Cross-validation: PASSED\n✅ Government verification: PASSED\n📧 Completion email with PDF sent",
                channel="document_upload"
            )
            print(f"[SUCCESS] ✅ Conversation logged")
        except Exception as log_error:
            print(f"[WARNING] ⚠️ Could not log conversation: {log_error}")
        
        # ============================================================================
        # ✅ STEP 8: RETURN SUCCESS
        # ============================================================================
        print(f"\n{'='*80}")
        print(f"[SUCCESS] 🎉 DOCUMENT CONFIRMATION COMPLETED SUCCESSFULLY")
        print(f"{'='*80}\n")
        
        return {
            "success": True,
            "message": "Document verification complete! Check your email for PDF summary.",
            "validation_passed": True,
...
@router.get("/get-extracted-data/{user_email}/{doc_type}")
def get_extracted_data(user_email: str, doc_type: str):
    ...
    try:
        ...
        # Map document types to validation fields
        field_map = {
            "emirates_id": "user_doc_validation",
            "ejari": "user_doc_validation",
            "trade_license": "user_doc_validation",
            "moa": "user_doc_validation"
        }
        
        field = field_map.get(doc_type)
        if not field or not user.get(field):
            return {}
        ...
        print(f"[ERROR] Get extracted data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    
from fastapi.responses import FileResponse
import mimetypes

@router.get("/get-document/{email}/{doc_type}")
async def get_document_file(
    ...
    """
    Serve document file for preview (inline display, not download)
    Path structure: backend/documents/id/{email}/{doc_type}/{filename}
    For members: backend/documents/id/{email}/{member_name}/{doc_type}/{filename}
    """
    try:
        print(f"\n[DEBUG] 🔍 get_document_file called")
        print(f"[DEBUG] Email: {email}")
        print(f"[DEBUG] Doc Type: {doc_type}")
        print(f"[DEBUG] Member Name: {member_name}")
        ...
        if member_name:
            search_path = base_path / member_name / doc_type
        else:
            search_path = base_path / doc_type
        
        print(f"[DEBUG] 🔍 Searching in: {search_path}")
        
        if not search_path.exists():
            print(f"[ERROR] ❌ Search path does not exist: {search_path}")
            raise HTTPException(status_code=404, detail=f"Document folder not found: {doc_type}")
        
        # Find the first file in the folder (should only be one document file + output.json)
        files = list(search_path.glob("*"))
        document_files = [f for f in files if f.is_file() and f.name != "output.json"]
        
        print(f"[DEBUG] 📄 Files found: {[f.name for f in files]}")
        print(f"[DEBUG] 📄 Document files (excluding output.json): {[f.name for f in document_files]}")
        
        if not document_files:
            print(f"[ERROR] ❌ No document file found in {search_path}")
            raise HTTPException(status_code=404, detail="No document file found")
        
        document_file = document_files[0]
        
        print(f"[DEBUG] ✅ Found document: {document_file.name}")
        print(f"[DEBUG] 📂 Full path: {document_file}")
        
        # Determine MIME type
        mime_type, _ = mimetypes.guess_type(str(document_file))
        if not mime_type:
            # Default MIME types
            if document_file.suffix.lower() == '.pdf':
                mime_type = 'application/pdf'
            elif document_file.suffix.lower() in ['.jpg', '.jpeg']:
                mime_type = 'image/jpeg'
            elif document_file.suffix.lower() == '.png':
                mime_type = 'image/png'
            elif document_file.suffix.lower() == '.webp':
                mime_type = 'image/webp'
            else:
                mime_type = 'application/octet-stream'
        
        print(f"[DEBUG] 📝 MIME type: {mime_type}")
        print(f"[SUCCESS] ✅ Serving document for preview: {document_file.name}")
        
        # ✅ KEY FIX: Use "inline" to display in browser, not "attachment" which forces download
        return FileResponse(
            path=str(document_file),
            media_type=mime_type,
            filename=document_file.name,
            headers={
                "Content-Disposition": f'inline; filename="{document_file.name}"',  # ✅ Changed from attachment
        ...
        print(f"[ERROR] ❌ Failed to serve document: {e}")
        import traceback
        traceback.print_exc()
...
async def download_document_file(
    ...
    """
    try:
        print(f"\n[DEBUG] 📥 download_document_file called")
        print(f"[DEBUG] Email: {email}")
        print(f"[DEBUG] Doc Type: {doc_type}")
        print(f"[DEBUG] Member Name: {member_name}")
        ...
        print(f"[DEBUG] ✅ Found document for download: {document_file.name}")
        ...
        print(f"[SUCCESS] ✅ Downloading: {document_file.name}")
        ...
        return FileResponse(
            ...
            filename=document_file.name,
            headers={
                "Content-Disposition": f'attachment; filename="{document_file.name}"',
                "Access-Control-Allow-Origin": "*"
            }
        )
