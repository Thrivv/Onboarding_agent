...
#                 time.sleep(5)  # wait 5 minutes between OCR calls to avoid rate limits
...
def check_documents_in_ocr(ocr_results: dict) -> dict:
    """
    Checks OCR results for presence of all 3 required documents.
    ocr_results: dict with format {'filename': {'type': 'commercial'/'eid'/'tenancy'/'unknown', 'text': 'extracted_text'}}
    Returns a dict: {'commercial': bool, 'eid': bool, 'tenancy': bool, 'missing': list}
    """
    has_commercial = False
    has_eid = False
    has_tenancy = False
    ...
    if not has_eid:
        missing.append("Resident Identity Card (EID)")
    if not has_tenancy:
        missing.append("Tenancy Contract (Ejari)")
...
def process_user_reply(from_email: str, body: str, attachments: list = None):
    ...
    if attachments:
        ...
        for a in attachments:
            filename = a["filename"]
            filedata = a["data"]
            # Support all file types now
            supported_extensions = [
                ".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp", ".tiff",  # Images
        ...
        for idx, filename in enumerate(files_saved):
            ...
            try:
                process_document(from_email, document_id, file_path, model_name=model_name)
                time.sleep(5)  # wait 5 seconds between OCR calls to avoid rate limits
            except Exception as e:
                print(f"[ERROR] OCR failed for {filename}: {e}")

        # Now extract structured OCR results
        try:
            # Gather OCR results from output.json files for each file
            ocr_results = {}
            ...
            if wrong_docs:
                for filename, doc_type in wrong_docs:
                    ...
                    if not doc_status["tenancy"]:
                        required_docs.append("Tenancy Contract (Ejari)")
                    send_wrong_document_email(from_email, filename, summary, required_docs)
                    
                    # Delete wrong document and its OCR results after sending mail
                    file_path = os.path.join("backend", "documents", "id", from_email, filename)
            ...
            # Check if all 3 documents are present and valid
            if not doc_status["missing"] and not wrong_docs:
                # Check for missing fields in validated documents
                missing_fields_msgs = []
                for doc_name, doc_info in ocr_results.items():
                    if doc_info.get("type") in ["commercial", "eid", "tenancy"]:
                        validation = doc_info.get("validation", {})
                        if not validation.get("is_valid", False):
                            missing_fields = validation.get("missing_fields", [])
                            if missing_fields:
                ...
                if missing_fields_msgs:
                    ...
                    body = (
                        "<html><body style='font-family:Arial,sans-serif;'>"
                        "<div style='max-width:600px;margin:auto;padding:24px;background:#fff;border-radius:10px;box-shadow:0 2px 8px #eee;'>"
                        "<h2 style='color:#4CAF50;'>All Documents Verified</h2>"
                        "<p>Dear User,</p>"
            ...
            if doc_status["missing"]:
                ...
                body = (
                    ...
                    "<p>We have received your submission. However, the following document(s) are still required:</p>"
                    f"<ul>{missing_list}</ul>"
                    "<p>Please reply to this email with the missing document(s) attached as files (PDF, DOC, DOCX, TXT, or image formats).</p>"
                    "<p style='margin-top:32px;'>Best regards,<br><strong>Thrivv Onboarding Team</strong></p>"
                    "</div></body></html>"
                )
                send_email(to_email=from_email, subject=subject, body=body, html=True)
                ...
                if wrong_docs:
                    ...
                    # Add LLM summaries
                    body += "\n".join(summary_texts)
                    body += "\nYou need to submit: Commercial Registration, Emirates ID, and Tenancy Contract documents.\n"
                body += "\nPlease reply to this email with the correct document(s) attached as files."
            ...
            if doc_status["missing"] or wrong_docs:
                ...
                for filename, _ in wrong_docs:
                    ...
                    doc_id = os.path.splitext(filename)[0]
                    ocr_dir = os.path.join("backend", "documents", "id", from_email, doc_id)
                    print(f"[DEBUG] Checking to delete OCR dir: {ocr_dir},{file_path}")
                    if os.path.exists(ocr_dir):
                        import shutil
                        shutil.rmtree(ocr_dir)
                    print(f"[INFO] Deleted wrong document and OCR results: {filename}")
            ...
            print(f"[ERROR] OCR failed for {from_email}: {e}")
            subject = "Error Processing Your Documents"
            body = f"We encountered an error while processing your documents. Please ensure your files are clear and readable, then try submitting them again.\n\nError details: {str(e)}"
            send_email(to_email=from_email, subject=subject, body=body)
            return
