...
Your response:"""
    ...
    try:
        ...
        if doc_type.lower() == "eid":
            ack = f"Got it ✓ - I've received your Emirates ID{member_context}. Let me extract the details."
...
def generate_dynamic_invalid_document_response(
    ...
    required_docs_formatted = "\n• ".join(allowed_info["required_docs"])
    remaining_docs_formatted = "\n• ".join(remaining_docs) if remaining_docs else "None"
...
def delete_invalid_document(
    ...
    try:
        ...
        if not doc_folder.exists():
            print(f"[WARN] Document folder does not exist: {doc_folder}")
            return False
        
        print(f"[INFO] 🗑️ Deleting invalid {doc_type.upper()} document: {doc_folder}")
...
#     ✅ FIXED: Properly includes member documents
#     """
#     documents = []
...
#         print(f"[DEBUG] 📂 Scanning documents in: {base_path}")
...
#                 documents.append(doc_info)
#                 print(f"[DEBUG] ✅ Added document: {doc_type} (valid={is_valid})")
                
#             except Exception as e:
#                 print(f"[ERROR] Reading {output_file}: {e}")
        
#         # ============================================================================
#         # ✅ MEMBER DOCUMENTS - SCAN ALL MEMBER FOLDERS
#         # ============================================================================
#         print(f"\n[DEBUG] 👥 Checking for member documents...")
        
#         # Load member progress to get member names
#         mp = load_member_progress(email)
#         if mp:
#             member_names = mp.get("members", [])
#             print(f"[DEBUG] Found {len(member_names)} members: {member_names}")
            
#             for member_name in member_names:
#                 # ✅ CRITICAL FIX: Member documents are stored directly under user path
#                 # NOT in a "members" subfolder (based on your code structure)
#                 member_dir = base_path / member_name
                
#                 print(f"\n[DEBUG] 📁 Checking member: {member_name}")
#                 print(f"[DEBUG] Member directory: {member_dir}")
#                 print(f"[DEBUG] Directory exists: {member_dir.exists()}")
                
#                 if not member_dir.exists():
#                     print(f"[WARN] ⚠️ Member directory not found: {member_dir}")
#                     continue
                
#                 # List contents
#                 try:
#                     items = list(member_dir.iterdir())
#                     print(f"[DEBUG] Items in {member_name} folder: {[item.name for item in items]}")
#                 except Exception as e:
#                     print(f"[ERROR] Cannot list directory: {e}")
#                     continue
                
#                 # Scan for document folders inside member directory
#                 for doc_folder in member_dir.iterdir():
#                     if not doc_folder.is_dir():
#                         continue
                    
#                     output_file = doc_folder / "output.json"
#                     print(f"[DEBUG] Checking: {doc_folder.name}/output.json")
                    
#                     if not output_file.exists():
#                         print(f"[DEBUG] ⏭️ No output.json in {doc_folder.name}")
#                         continue
...
#                             "submitted": True,
#                             "is_valid": is_valid,
#                             "extracted_data": data.get("extracted_fields", {}) or data.get("extracted_data", {}),
#                             "validation": data.get("validation", {}),
#                             "member_name": member_name  # ✅ CRITICAL: Include member name
#                         }
                        
#                         documents.append(doc_info)
#                         print(f"[DEBUG] ✅ Added member document: {member_name} - {doc_type} (valid={is_valid})")
...
#         else:
#             print(f"[DEBUG] ℹ️ No member progress found (not a partnership account)")
...
#     print(f"\n[INFO] 📊 Total documents found: {len(documents)}")
#     print(f"[INFO] Documents breakdown:")
#     main_docs = [d for d in documents if not d.get("member_name")]
#     member_docs = [d for d in documents if d.get("member_name")]
#     print(f"  - Main user documents: {len(main_docs)}")
#     print(f"  - Member documents: {len(member_docs)}")
    
#     return documents
# ============================================================================
# In chatupload.py - ONLY UPDATE THIS FUNCTION
# ============================================================================
# In chatupload.py - UPDATE THIS FUNCTION (around line 800)
...
def get_submitted_documents(email: str) -> List[dict]:
    ...
    try:
        ...
        for item in base_path.iterdir():
            ...
            print(f"\n[DEBUG] 🔍 Checking folder: {item.name}")
            print(f"[DEBUG]    output.json exists: {output_file.exists()}")
            
            if not output_file.exists():
                print(f"[DEBUG] ⭕ Skipping {item.name} - no output.json")
                continue
            ...
            try:
                ...
                if "extracted_fields" in data:
                    ...
                    print(f"[DEBUG] 📊 Fields: {list(extracted.keys()) if isinstance(extracted, dict) else 'Not a dict'}")
                ...
                if not extracted and "result" in data:
                    ...
                    if isinstance(result, dict):
                        if "extracted_fields" in result:
                            ...
                            print(f"[DEBUG] ✅ Found extracted_fields in RESULT object")
                            print(f"[DEBUG] 📊 Fields: {list(extracted.keys()) if isinstance(extracted, dict) else 'Not a dict'}")
                        elif "extracted_data" in result:
                            extracted = result["extracted_data"]
                            print(f"[DEBUG] ✅ Found extracted_data in RESULT object")
                            print(f"[DEBUG] 📊 Fields: {list(extracted.keys()) if isinstance(extracted, dict) else 'Not a dict'}")
                ...
                if not extracted or not isinstance(extracted, dict):
                    ...
                    print(f"[DEBUG] 🔎 Full JSON structure (first 500 chars):")
                ...
                doc_info = {
                    ...
                    "submitted": True,
                    "is_valid": is_valid,
                    "extracted_data": extracted,  # ✅ Should be populated
                    "validation": validation,
                    "member_name": None  # Main user document
                }
                
                documents.append(doc_info)
                print(f"[DEBUG] ✅ Added document: {doc_type} (valid={is_valid}, fields={len(extracted)})")
                print(f"[DEBUG] 📊 Extracted fields preview: {list(extracted.keys())[:5] if extracted else 'None'}")
        ...
        # ============================================================================
        print(f"\n{'='*80}")
        print(f"[DEBUG] 👥 Checking for member documents...")
        print(f"{'='*80}\n")
        ...
        if mp:
            ...
            for member_name in member_names:
                ...
                print(f"\n[DEBUG] 🔍 Checking member: {member_name}")
                print(f"[DEBUG] Directory: {member_dir}")
                print(f"[DEBUG] Exists: {member_dir.exists()}")
                
                if not member_dir.exists():
                    print(f"[WARN] ⚠️ Member directory not found")
                    continue
                ...
                for doc_folder in member_dir.iterdir():
                    ...
                    output_file = doc_folder / "output.json"
                    print(f"[DEBUG] Checking: {doc_folder.name}/output.json - exists: {output_file.exists()}")
                    ...
                    try:
                        ...
                        documents.append(doc_info)
                        print(f"[DEBUG] ✅ Added member doc: {member_name} - {doc_type} (fields={len(extracted)})")
            ...
            print(f"[DEBUG] ℹ️ No member progress found")
    ...
    main_docs = [d for d in documents if not d.get("member_name")]
    member_docs = [d for d in documents if d.get("member_name")]
    print(f"[INFO] Main user: {len(main_docs)}")
    print(f"[INFO] Members: {len(member_docs)}")
    
    # ✅ PRINT SAMPLE OF EXTRACTED DATA
    if documents:
        print(f"\n[DEBUG] 📋 SAMPLE DOCUMENT DATA:")
        sample = documents[0]
        print(f"  - Type: {sample.get('document_type')}")
        print(f"  - Valid: {sample.get('is_valid')}")
        print(f"  - Extracted fields: {list(sample.get('extracted_data', {}).keys())}")
        if sample.get('extracted_data'):
            print(f"  - Sample values: {list(sample['extracted_data'].items())[:3]}")
...
def process_uploaded_document(email: str, uploaded_file: UploadFile, member_name: Optional[str]) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # ✅ SIMPLE FIX: Set up paths with guaranteed directory creation
            # # ✅ FIXED CODE
            # if is_member:
            ...
            #     # ✅ Create member directory BEFORE creating document folder
            #     member_dir = base_user_dir / member_name_clean
            #     member_dir.mkdir(parents=True, exist_ok=True)
                
            #     save_dir = member_dir / document_id
            # else:
            #     base_user_dir = user_docs_path(email)
            #     base_user_dir.mkdir(parents=True, exist_ok=True)
            ...
            # # ✅ NOW create the final save directory
            ...
            if doc_type == "unknown":
                print(f"[WARN] Unknown document type detected")
            ...
            if is_member and doc_type != "eid":
                print(f"[WARN] Wrong document type for member (expected EID, got {doc_type})")
...
def format_extraction_message(doc_type: str, extracted_data: dict, member_name: Optional[str]) -> str:
    ...
    if doc_type.lower() == "eid":
        ...
        msg += "Please verify to confirm the above details or let me know if something is incorrect."
        return msg

    # 🏢 COMMERCIAL LICENSE DETAILS
    elif doc_type.lower() == "commercial":
        msg = "✅ Commercial License processed successfully.\n\n"
        msg += "### 🏛️ Company Information\n"
        ...
        msg += "Please verify to confirm the above details."
        return msg

    # 🏘️ EJARI / TENANCY DETAILS
    elif doc_type.lower() in ("ejari", "tenancy"):
...
# ============================================================================
# UPDATED handle_verification_response FUNCTION
# ============================================================================
...
def handle_verification_response(email: str, user_message: str, user_data: dict) -> dict:
    ...
    UPDATED: 
    - Accept subtle keywords: "ok", "okay", "sure", "verify", "verified", "confirm", etc.
    - Remove green box from chat messages (shown only in email)
    - Update completion message to "Document verification stage completed"
    """
            ...
            if progress_file.exists():
                try:
                    ...
                    if verified_count >= total_members:
                        ...
                        # ✅ UPDATED: Changed message to "Document verification stage completed"
                        # ✅ NOTE: No green box displayed in chat (shown only in email)
                        next_message = f"✅ {latest_member_name}'s EID verified!\n\n🎉 Document verification stage completed! All {total_members} member EIDs have been verified.\n\nYour partnership onboarding is complete! Your account will be activated within 3-4 business days."
...
def insert_conversation_log(log_data: dict, channel: str = "chat") -> bool:
    """
    Insert conversation log into conversations table with channel
    channel can be: 'chat', 'email', 'web', etc.
    """
    try:
        supabase.table("conversations").insert(
            {
                ...
                "role": log_data["role"],
                "message": log_data["message"],
                "channel": channel,  # NEW: Distinguish between chat and email
                "timestamp": (
                    log_data["timestamp"].isoformat()
                    if isinstance(log_data["timestamp"], datetime)
                    else log_data["timestamp"]
...
async def start_conversation(request: StartConversationRequest):
    ...
    # Log initial message WITH CHANNEL
    try:
        ...
        }, channel="chat")  # Mark as chat channel
    except:
        pass
...
async def upload_conversational(
    ...
    # If processing failed (unknown doc, technical error), return early
    ...
    # ============================================================================
    # STEP 3: HANDLE WRONG DOCUMENT TYPE (WITH DYNAMIC RESPONSE)
    # ============================================================================
    if not is_valid_type:
        ...
        return {
            ...
            "remaining_documents": remaining_docs,  # ✅ NEW: Remaining docs list
            "document_file_path": None
        }
    ...
    if result["success"] and result["is_valid"]:
        # ✅ SEND DYNAMIC EMAIL WITH REMAINING DOCS
        # send_document_verification_email_dynamic(
        #     to_email=email,
        #     user_name=user_name,
        #     doc_type=result["document_type"],
        ...
        #     member_name=clean_member_name
        # )
        print(f"[INFO] ✅ Dynamic verification email sent for {result['document_type']}")
    else:
        # Document failed validation - send AI reasoning email
        if "ai_reasoning" in result:
            send_ai_reasoning_email(
                to_email=email,
                user_name=user_name,
            ...
            print(f"[INFO] ✅ AI reasoning email sent")
        else:
    ...
    # Combine dynamic acknowledgment with extracted data
    ...
    # ✅ RETURN SUCCESS RESPONSE WITH DYNAMIC CONTENT
    return {
        "success": result["success"],
        "message": complete_message,  # ✅ Dynamic AI-generated message
        "acknowledgment": dynamic_acknowledgment,  # ✅ Separate acknowledgment
        ...
        "remaining_documents": remaining_docs,  # ✅ NEW: List of remaining docs
        "wrong_document_type": False
    }
...
def generate_ai_reasoning_for_invalid_document(
    ...
    try:
        ...
        print(f"[INFO] 🤖 Generating AI reasoning for invalid {doc_type} document")
...
Now generate your response for this user:"""
        ...
        if not reasoning or len(reasoning) < 50:
            print("[WARN] LLM returned poor response, using fallback")
...
def send_ai_reasoning_email(
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>⚠️ Document Validation Issue</h1>
            </div>
            <div class="content">
                <p>Dear {user_name},</p>
