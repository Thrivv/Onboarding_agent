...
SPECIALIZED_DOC_TYPES = {
    "commercial": ["commercial registration", "trade license", "business license", "رخصة تجارية", "license no"],
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if any(code in error_msg for code in ['503', '429', '502', '504']) and attempt < max_retries - 1:
                        ...
                        print(f"[WARN] API error (attempt {attempt + 1}/{max_retries}): {error_msg}")
...
def run_ocr(image_path: str, model_name: str = LLAMA_MODEL_NAME) -> str:
    ...
    try:
        ...
        if response.status_code in [502, 503, 504]:
            raise RuntimeError(f"OCR failed: {response.status_code} Server temporarily unavailable")
        
        if response.status_code == 429:
            raise RuntimeError(f"OCR failed: {response.status_code} Rate limit exceeded")
        ...
        raise RuntimeError(f"OCR failed: {response.status_code if 'response' in locals() else 'Network error'} {str(e)}")
...
def process_document(user_email: str, document_id: str, file_path: str, model_name: str = LLAMA_MODEL_NAME, is_member: bool = False) -> dict:
    """
    ...
    try:
        ...
        if is_member and doc_type != "eid":
            analysis = {
                ...
                "raw_text": "",
                "extracted_fields": {},
                "validation": {"is_valid": False, "required_fields": [], "present_fields": [], "missing_fields": ["EID"]},
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if attempt == 1:
                ...
                current_model = QWEN_MODEL_NAME if model_name == LLAMA_MODEL_NAME else LLAMA_MODEL_NAME
            else:
            ...
            if doc_type == "commercial":
                ...
                if len(result) == 3:
                    ...
                    validation_result = extractor.validate_extraction(english_data, arabic_data, owner, ownership_type="Single Owner")
                    ...
                    validation_result = extractor.validate_extraction(english_data, arabic_data, managers, partners, ownership_type="Multiple Owners")
                    ...
                    analysis = {
                        ...
                        "validation": validation_result,
                        "status_message": "✅ VALID: Multiple Owners Commercial License extracted" if validation_result["is_valid"] else f"❌ INCOMPLETE: {', '.join(validation_result.get('issues', []))}",
                        "is_valid": validation_result["is_valid"]
                    }
            ...
            # ====================
                ...
                english_data, arabic_data = extractor.extract_with_ai(raw_text, model_choice="llama" if "llama" in current_model else "qwen")
                validation_result = extractor.validate_extraction(english_data, arabic_data)
                ...
                analysis = {
                    ...
                    "validation": validation_result,
                    "status_message": "✅ VALID: Ejari extracted" if validation_result["is_valid"] else f"❌ INCOMPLETE: {', '.join(validation_result.get('issues', []))}",
                    "is_valid": validation_result["is_valid"]
                }
            ...
            # ====================
                ...
                analysis = {
                    ...
                    "validation": validation_result,
                    "status_message": "✅ VALID: MOA extracted" if validation_result["is_valid"] else f"❌ INCOMPLETE: {', '.join(validation_result.get('issues', []))}",
                    "is_valid": validation_result["is_valid"]
                }
            ...
            # ====================
                ...
                analysis = {
                    ...
                    "status_message": "✅ VALID: EID extracted" if validation["is_valid"] else f"❌ INCOMPLETE: missing {', '.join(validation['missing_fields'])}",
