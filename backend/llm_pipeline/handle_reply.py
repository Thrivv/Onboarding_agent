...
# def generate_reasoning_for_invalid_document(
#     raw_text: str, 
#     doc_type: str, 
...
# ) -> str:
#     """
#     Generate detailed reasoning for why a document is invalid using OpenAI.
#     Compares extraction logs, validation results, and expected fields.
    
#     Args:
#         raw_text: The OCR/extracted text from the document
#         doc_type: Type of document (eid, commercial, ejari, moa, etc.)
#         validation_result: Dictionary containing validation details
#         extracted_fields: Fields that were successfully extracted
#         expected_fields: Fields that were expected but missing
#     """
#     try:
#         from llm_runner.run_model import call_local_llm
...
#         prompt = f"""You are an expert document validation assistant. A user uploaded a document that failed validation.

# {full_context}

# Based on the above information, provide a clear, professional, and helpful explanation of why this document is invalid. Your response should:

# 1. Identify the specific issues (missing fields, incorrect format, unreadable sections, etc.)
# 2. Explain what was expected vs what was found
# 3. Provide actionable guidance on what the user needs to do to fix the issue
# 4. Be empathetic and professional in tone

# Format your response as a clear, conversational explanation (not bullet points). Keep it concise but thorough (3-5 sentences).

# Example tone: "I analyzed your {doc_type.upper()} document and found that it's missing critical information. Specifically, the following required fields could not be extracted: [fields]. This usually happens when the document image is unclear or the format is non-standard. Please upload a clear, high-quality scan or photo of your {doc_type.upper()} ensuring all text is readable."
...
#             # Fallback if OpenAI returns poor response
#             reasoning = generate_fallback_reasoning(doc_type, validation_result)
        
#         return reasoning
        
#     except Exception as e:
#         print(f"[ERROR] Failed to generate reasoning for invalid document: {e}")
#         import traceback
#         traceback.print_exc()
#         return generate_fallback_reasoning(doc_type, validation_result)


# def generate_fallback_reasoning(doc_type: str, validation_result: dict = None) -> str:
#     """Generate fallback reasoning if OpenAI fails"""
#     missing = []
#     if validation_result:
#         missing = validation_result.get('missing_fields', [])
...
#         return (
#             f"Your {doc_type.upper()} document could not be validated because the following required "
#             f"fields are missing or unclear: {fields_list}. Please upload a clear, complete copy of "
#             f"your {doc_type.upper()} document with all information visible and readable."
#         )
#     else:
#         return (
#             f"Your {doc_type.upper()} document could not be validated. This may be due to poor image "
#             f"quality, incorrect document format, or missing required information. Please ensure you "
#             f"upload a clear, high-quality scan or photo of the correct document type."
#         )
...
#     """
#     Send an email with the reasoning for why the document is invalid.
#     Enhanced with better formatting and member support.
#     """
#     from email_utils import send_email
    
#     member_info = f" for {member_name}" if member_name else ""
#     subject = f"⚠️ Document Upload Issue{member_info}: {filename}"
...
#             .alert-box {{ background: #fff3cd; border-left: 4px solid #ffc107; padding: 16px; margin: 20px 0; border-radius: 6px; }}
#             .alert-box strong {{ color: #856404; display: block; margin-bottom: 8px; }}
#             .reasoning-box {{ background: #f8f9fa; border: 1px solid #dee2e6; padding: 20px; margin: 20px 0; border-radius: 8px; line-height: 1.6; }}
#             .reasoning-box p {{ margin: 0; color: #495057; font-size: 15px; }}
#             .doc-info {{ background: #e3f2fd; padding: 15px; border-radius: 6px; margin: 20px 0; }}
#             .doc-info p {{ margin: 5px 0; color: #1976d2; font-weight: 500; }}
#             .action-section {{ background: #e8f5e9; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center; }}
#             .action-section p {{ margin: 10px 0; color: #2e7d32; }}
...
#                 <h2>Document Validation Issue</h2>
#             </div>
            
#             <div class="content">
#                 <p>Dear User,</p>
#                 <p style="margin-top: 15px;">We attempted to process your document but encountered validation issues that prevent us from accepting it.</p>
                
#                 <div class="doc-info">
#                     <p><strong>📄 Document:</strong> {filename}</p>
#                     <p><strong>📋 Type:</strong> {doc_type.upper()}</p>
#                     {f'<p><strong>👤 For:</strong> {member_name}</p>' if member_name else ''}
#                 </div>
                
#                 <div class="alert-box">
#                     <strong>⚠️ Why was this rejected?</strong>
#                 </div>
...
#                     <p><strong>✅ Next Steps:</strong></p>
#                     <p>Please review the issue above and re-upload a corrected version of your {doc_type.upper()} document.</p>
#                     <p style="margin-top: 15px; font-size: 14px;">💡 <em>Tip: Ensure the document is clear, complete, and all text is readable before uploading.</em></p>
#                 </div>
                
#                 <p style="margin-top: 25px; color: #6c757d;">If you have questions or need assistance, please reply to this email.</p>
#             </div>
            
#             <div class="footer">
#                 <p><strong>Thrivv Onboarding Team</strong></p>
#                 <p>Automated Document Verification System</p>
#             </div>
#         </div>
#     </body>
#     </html>
...
#         print(f"[INFO] ✅ Sent reasoning email for invalid document: {filename} to {to_email}")
#     except Exception as e:
#         print(f"[ERROR] Failed to send reasoning email: {e}")
        
# def format_invalid_document_response(invalid_docs: list) -> str:
#     """
#     Format invalid documents with reasoning for chat display
#     """
#     if not invalid_docs:
#         return ""
    
#     messages = []
#     messages.append("⚠️ **Document Validation Issues**\n")
    
#     for doc in invalid_docs:
#         filename = doc.get("filename", "Unknown")
#         doc_type = doc.get("type", "unknown").upper()
#         reasoning = doc.get("reasoning", "The document could not be validated.")
        
#         messages.append(f"**{filename}** ({doc_type})")
#         messages.append(f"_{reasoning}_\n")
    
#     messages.append("📧 **Detailed explanations have been sent to your email.**")
#     messages.append("\nPlease re-upload corrected versions of these documents to continue.")
...
def send_completion_email(to_email: str, account_type: str):
    """
    Send onboarding completion email with PDF summary attached
    FIXED: Corrected path construction to find documents directory
    """
    # Fetch user details
    user_response = supabase.table("users").select("*").eq("email", to_email).execute()
    user = user_response.data[0] if user_response.data else {}
    ...
    # handle_reply.py is at: .../backend/llm_pipeline/handle_reply.py
    # Documents are at:      .../backend/documents/id/{email}/
    ...
    # Define project_root in case we need it for alternative paths
    project_root = os.path.dirname(code_backend_dir)
    # Result: .../Onboarding_agent-Srini-Multiple-Owners-Logic/
    
    # Documents are in the SAME backend folder
    ...
    if os.path.exists(user_docs_dir):
        items = os.listdir(user_docs_dir)
        print(f"[DEBUG] Items in user docs ({len(items)}): {items}")
        ...
        for item in items:
            ...
            if os.path.isdir(item_path):
                output_path = os.path.join(item_path, "output.json")
                if os.path.exists(output_path):
                    print(f"[DEBUG] ✓ Found output.json in: {item}/")
                else:
                    print(f"[DEBUG]  No output.json in: {item}/")
    else:
        print(f"[DEBUG] ❌ User docs directory does NOT exist!")
        print(f"[DEBUG] Attempting to find documents...")
        
        # Try alternative paths
        alt_paths = [
            os.path.join(code_backend_dir, "documents", "id", to_email),
            os.path.join(project_root, "backend", "documents", "id", to_email),
            os.path.join("backend", "documents", "id", to_email),
        ]
        
        for alt_path in alt_paths:
            print(f"[DEBUG] Trying: {alt_path}")
            if os.path.exists(alt_path):
                print(f"[DEBUG] ✓ FOUND DOCUMENTS AT: {alt_path}")
                user_docs_dir = alt_path
                break
    ...
    try:
        generate_user_summary_pdf(user, user_docs_dir, pdf_path)
        print(f"[INFO] PDF Summary generated successfully: {pdf_path}")
        
        # Verify PDF was created and has content
        if os.path.exists(pdf_path):
            pdf_size = os.path.getsize(pdf_path)
            print(f"[INFO] PDF file size: {pdf_size} bytes")
            if pdf_size < 1000:
...
#     # FIXED PATH CONSTRUCTION
#     # handle_reply.py is at: .../Onboarding_agent-Srini-Multiple-Owners-Logic/Onboarding_agent-Srini-Multiple-Owners-Logic/backend/llm_pipeline/handle_reply.py
#     # Documents are at:      .../Onboarding_agent-Srini-Multiple-Owners-Logic/Onboarding_agent-Srini-Multiple-Owners-Logic/backend/documents/id/{email}/
...
#     # Email body (rest remains the same)
#     subject = "Onboarding Complete!"
#     body_html = f"""
#     <html><body style='font-family:Arial,sans-serif;color:#333;'>
#         <div style='max-width:600px;margin:auto;padding:24px;background:#fff;border-radius:10px;box-shadow:0 2px 8px #eee;'>
    ...
    try:
        ...
        for member_name in members:
            ...
            if not os.path.exists(member_dir):
                print(f"[WARN] Member directory not found: {member_name}")
                continue
